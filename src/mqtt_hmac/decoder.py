"""Decoder for the modified MQTT/HMAC packet format."""
...
def decode_packet(data: bytes, key: Optional[bytes], signer: Optional[HmacSigner] = None) -> MqttHmacPacket:
    """Decode bytes into `MqttHmacPacket`.

    If `key` is provided, the decoder will verify the HMAC and raise
    `HmacVerificationError` on mismatch.
    """
    if signer is None:
        signer = HmacSha()
    ...
    if key is not None:
        to_verify = topic_bytes + payload
        if not signer.verify(key, to_verify, signature):
